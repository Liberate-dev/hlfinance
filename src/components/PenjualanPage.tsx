...
export default function PenjualanPage({ startInAddMode = false }: PenjualanPageProps) {
  ...
  const handleAddLine = () => {
    setFormLines((prev) => [...prev, { productId: '', qty: 1 }]);
  };
  ...
  const handleSave = async () => {
    ...
    if (formLines.length === 0) { notifyError('Minimal satu produk harus ditambahkan.'); return; }
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
              <div className="flex justify-between items-center">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 border border-slate-300 hover:border-[#002B8F] hover:bg-blue-50 text-slate-600 hover:text-[#002B8F] font-bold text-sm rounded-xl cursor-pointer"
                >
                  <Plus size={16} />
                  Tambah Produk
                </button>
  ...
  const renderDetail = () => {
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        ...
        <BonDetailView
          bon={selectedBon}
          customerAlamat={customers.find((c) => c.id === selectedBon.customer_id)?.alamat}
        />
      </div>
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        description="Bon batal akan disembunyikan dari daftar dan laporan. Data bisa dipulihkan lewat menu Admin."
      />
    </div>
  );
}
